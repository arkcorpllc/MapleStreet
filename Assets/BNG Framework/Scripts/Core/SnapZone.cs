    ...
    public class SnapZone : MonoBehaviour {
        ...
        public void GrabGrabbable(Grabbable grab) {
            ...
            if (pv != null)
            {
                if (proceedRpc)
                {
                    pv.RPC("GrabGrabbableRpc", RpcTarget.OthersBuffered, grab.gameObject.name);
                }
                else
                {
                    Debug.LogFormat("SnapZone::GrabGrabbable SnapZone {0} is marked for not further calling RPC", name);
                }
            }
            else
            {
                Debug.LogFormat("SnapZone::GrabGrabbable SnapZone {0} is local only, PhotonView is null", name);
            }
        ...
        public void ReleaseAll() {
            Debug.LogFormat("SnapZone - ReleaseAll - GO = {0}", gameObject);
            ...
            HeldItem = null;
            if (pv != null && proceedRpc)
            {
                if (proceedRpc)
                {
                    pv.RPC("ReleaseAllRpc", RpcTarget.OthersBuffered);
                }
                else
                {
                    Debug.LogFormat("SnapZone::GrabGrabbable SnapZone {0} is marked for not further calling RPC", name);
            ...
            {
                Debug.LogFormat("SnapZone::ReleaseAll - SnapZone {0} is local only, PhotonView is null", name);
            }
        }

        [PunRPC]
        public void GrabGrabbableRpc(string grabName)
        {
            proceedRpc = false;
            Debug.LogFormat("SnapZone::GrabGrabbableRpc zone {0}, item {1}", name, grabName);
            var snapItem = GameObject.Find(grabName).GetComponent<Grabbable>();
            GrabGrabbable(snapItem);
            proceedRpc = true;
        }

        [PunRPC]
        public void ReleaseAllRpc()
        {
            proceedRpc = false;
            Debug.LogFormat("SnapZone::ReleaseAllRpc zone {0}", name);
            ReleaseAll();
            proceedRpc = true;
        }
    }
