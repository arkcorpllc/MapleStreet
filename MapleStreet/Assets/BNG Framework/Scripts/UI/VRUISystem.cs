    ...
    public class VRUISystem : BaseInputModule {
        ...
        void init() {
            if(cameraCaster == null) {
				...
				// Modified -- UI hack to fix button shifting after scene transition
				var foundCaster = GameObject.Find("CameraCaster");
				if (foundCaster)
				{
					cameraCaster = foundCaster.GetComponent<Camera>();
					return;
				}
				// Modified -- UI hack to fix button shifting after scene transition
